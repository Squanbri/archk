...
{ 
  ...
  {
    if ($table === '') {
      $user = Db::table('squanbri_authentication_users')->where('email', $email)->first();
      $company = Db::table('squanbri_authentication_companies')->where('email', $email)->first();
  ...
  public function authenticate($data) 
  {
    ...
    { 
      ...
      { 
        if ($this->findUser($data['table'], $data['email'])['activate'] === 1) 
        {
